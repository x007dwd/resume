...
\begin{itemize}\small
  \item 将RGB图像转换至HSV空间，在HSV的三个通道分别设置阈值，检测小球的位置。
  \item 使用Kalman滤波器对跟踪的位置、误差进行预测和跟新。
\end{itemize}
...
\begin{itemize}\small
  ...
  \item 使用 Lucene 提供的 api 构建索引库, 前端使用 jsp 接收用户查询,在后台使用 servlet 对用户查询进 行分词处理,之后到索引库中进行文档匹配, 最后把查询结果集反馈给用户并在前端页面中 展示。
\end{itemize}
